...
// export 'package:interactive_slider/interactive_slider_co`ntroller.dart';
...
class MInteractiveSlider extends StatefulWidget {
  ...
  const MInteractiveSlider({
    ...
    this.transitionCurvePeriod = MInteractiveSlider.defaultTransitionPeriod,
  ...
  @override
  State<MInteractiveSlider> createState() => _InteractiveSliderState();
}

class _InteractiveSliderState extends State<MInteractiveSlider> {
  ...
  @override
  void didUpdateWidget(MInteractiveSlider oldWidget) {
          ...
          children: [
            Container(
              ...
              decoration: ShapeDecoration(
                ...
                color:
                    widget.backgroundColor ?? brightnessColor.withOpacity(0.12),
              ),
            ),
            AnimatedContainer(
              clipBehavior: Clip.antiAlias,
              width: double.infinity,
              height: _height.value + 10,
                    ...
                    child: ValueListenableBuilder<EdgeInsets>(
                      ...
                      builder: (context, margin, child) {
                        return AnimatedPadding(
                          duration: widget.transitionDuration,
                          curve: _transitionCurve,
                          padding: EdgeInsets.zero, // margin,
                          child: child,
                        );
                      },
                    ),
...
class InteractiveSliderPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final progressRect =
        Rect.fromLTWH(0, 5, progress.value * size.width, size.height - 10);
    final progressRRect = RRect.fromRectAndCorners(progressRect,
    ...
    if (numberOfSegments case int numberOfSegments) {
      ...
      final sliderRect = RRect.fromRectAndRadius(
          Rect.fromLTWH(progress.value * size.width, 1, 15, size.height - 2),
          const Radius.circular(5));

      canvas.drawRRect(progressRRect, _rectPaint);
      canvas.drawRRect(
        progressRRect,
        Paint()
          ..style = PaintingStyle.stroke
      ...
      // Slider adjuster background color white
      canvas.drawRRect(
        sliderRect,
        Paint()
          ..color = Colors.white
      ...
      // Center line
      canvas.drawLine(
        Offset(progress.value * size.width + 7.5, 10),
        Offset(progress.value * size.width + 7.5, size.height - 10),
        Paint()
          ..color = Colors.black45
          ..style = PaintingStyle.fill
          ..strokeWidth = 1,
      ...
      TextPainter(
        text: TextSpan(
          text: "\$${(progress.value * 250).floor()}/mo",
          style: const TextStyle(color: Colors.black),
        ),
        textDirection: TextDirection.ltr,
      )
        ..layout(maxWidth: size.width)
        ..paint(canvas, Offset(10, size.height / 2 - 8));
    }
  }
